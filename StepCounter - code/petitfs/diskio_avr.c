...
#define SELECT() PORTB &= ~(1<<PORTB0) /* CS = L */
#define DESELECT() PORTB |= (1<<PORTB0) /* CS = H */
//#define SELECTING ((PORTB.DIR & (1<<PORTB6)) && !(PORTB.OUT & (1<<PORTB6)))

static void init_spi(void)
{
	
	DDRB |= (1<<PORTB1) | (1<<PORTB2) | (1<<PORTB0);	//SCKL, MOSI, CS outputi
	DDRB &= ~(1<<PORTB3);								//MISO input, Èe mora biti pull up ali ne je magija
	PORTB &= ~(1<<PORTB1) & ~(1<<PORTB2);				//SCKL, MOSI ugasnjena		
	PORTB |= (1<<PORTB0);								//CS na logièni enici
	///* Customize SPI prescaler to give 100-400kHz clock */
	SPCR |= (1<<SPE) | (1<<MSTR) | (1<<SPR1);			//Prescale = Fcpu/64
}
